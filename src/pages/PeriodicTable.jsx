...
function ElementCell({ el, onClick, dimmed }) {
  ...
  return (
    <button onClick={() => onClick(el)} title={`${el.name} (${el.symbol})`}
      style={{
        width: '100%', height: '100%', background: bg, color: '#f0f0f0',
        border: '1px solid rgba(255,255,255,0.07)', borderRadius: 4, padding: '3px 2px',
        cursor: 'pointer', textAlign: 'center',
        transition: 'transform 0.12s ease, opacity 0.2s ease',
        opacity: dimmed ? 0.08 : 1,
        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
      }}
      onMouseEnter={e => { if (!dimmed) { e.currentTarget.style.transform = 'scale(1.25)'; e.currentTarget.style.zIndex = 20; e.currentTarget.style.position = 'relative'; e.currentTarget.style.boxShadow = '0 4px 16px rgba(0,0,0,0.5)' } }}
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: '1.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
          <button className="btn" onClick={() => setFilter('all')}
            style={{ fontSize: '0.72rem', padding: '4px 10px', background: filter === 'all' ? 'var(--text)' : 'var(--bg2)', color: filter === 'all' ? 'var(--bg)' : 'var(--text)' }}>Все</button>
          {categories.map(c => (
        ...
        <div onClick={() => setSelected(null)} style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
              ...
              <button onClick={() => setSelected(null)} style={{ background: 'none', border: '1px solid var(--border)', borderRadius: 8, width: 34, height: 34, cursor: 'pointer', color: 'var(--text2)', fontSize: '1.1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>&times;</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: '1.25rem' }}>
              {['Атомная масса', 'Период', 'Группа', 'Фаза', 'Открыт', 'Конфигурация'].map((k, i) => {
                const vals = [selected.mass, selected.period, selected.group,
                  selected.phase === 'solid' ? 'Твёрдое' : selected.phase === 'gas' ? 'Газ' : selected.phase === 'liquid' ? 'Жидкое' : selected.phase,
                  selected.discovered < 0 ? `${Math.abs(selected.discovered)} до н.э.` : selected.discovered,
                  selected.config]
                ...
                return (
                  <div key={k} style={{ background: 'var(--bg2)', borderRadius: 8, padding: '0.65rem 0.9rem' }}>
                    ...
                    <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.85rem', fontWeight: 500 }}>{vals[i]}</div>
                  </div>
                )
              })}
            </div>
            {selected.description && <p style={{ color: 'var(--text2)', fontSize: '0.85rem', lineHeight: 1.7 }}>{selected.description}</p>}
          </div>
        </div>
      )}
    </div>
